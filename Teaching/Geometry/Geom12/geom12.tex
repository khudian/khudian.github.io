...
where we denote by $R=\begin{pmatrix}
      ...
      0 & -1\\
       ...
       We see that in the second case the orthogonal matrix is composition of rotation and reflection matrix:
...
\end{equation}
  ...
  We come to proposition
        ...
        {\bf Proposition}. {\it Let $A$ be an arbitrary $2\times 2$ orthogonal matrix, i.e. $A^TA=1$
        and in particularly $\det A=\pm 1$. (Transition matrix transforms an  orthonormal basis
          to an orthonormal one.)

         If $\det A=1$
      then there exists an angle $\varphi\in [0,2\pi)$ such that
      $A=A_\varphi$ is a transition matrix \eqref{orthogonalrotation}
      which rotates the basis vectors on the  angle  $\varphi$.

      If $\det A=-1$ then there exists an angle $\varphi\in [0,2\pi)$ such that $A=\tilde A_\varphi$
      is a transition matrix is a composition of rotation and reflection (see \eqref{rotation+reflectioninaction}).
      
      {\it {\footnotesize One can show that in this case the transition matrix $\tilde A_\varphi$ is a reflection matrix with respect
...
\m
   ...
   {\small Let $(x,y)$ be components of the vector ${\ac}$ in the basis $(\e,\f)$,
   and $(x',y')$ be components of the vector $\ac$ in the rotated basis $\{\e',\f'\}$.

   Then  it follows from \eqref{rotationofbasisontheangle} that
                                 $$
   \ac= x'\e'+y'\f'=(\e',\f')\begin{pmatrix}x'\cr y'\end{pmatrix}=
         (\e,\f)A_\varphi\begin{pmatrix}x'\cr y'\end{pmatrix}=(\e,\f)\begin{pmatrix}x\cr y\end{pmatrix}\,.
                               $$
  Hence
              \begin{equation}\label{transformofcoord1}
     \begin{pmatrix}x\cr y\end{pmatrix}=A_{\varphi}\begin{pmatrix}x'\cr y'\end{pmatrix}=
        \begin{pmatrix}
...
\subsection {Orientation in vector space}
   ...
   In the three-dimensional Euclidean space except scalar (inner) product, one can consider
   another important operation: vector product.  For defining this operation we need
   additional structure:{\it orientation}.

     A basis  $\{\ac,\b,\c\}$ have the same orientation as the basis $\{\ac',\b',\c'\}$
...
\bigskip
  ...
  How to make this conception more mathematical?
...
Consider the set of {\it all} bases in the  given vector space  $V$.
 ...
 If $(\e_1,\dots\e_n)$, $(\e'_1,\dots\e'_n)$ are two bases then one can consider the matrix $T$---transition matrix
  which transforms the old basis to the new one (see \eqref{transformoforthogbasis}).
  The transition matrix is not degenerate, i.e. determinant of this matrix is not equal to zero.





  {\bf Definition} Let $\{\e_1,\dots\e_n\}, \{\e'_1,\dots\e'_n\}\in \R^n$ be two bases in $\R^n$ and
  $T$ be transition matrix:
\begin{equation}\label{transitionmatrix1}
    \{\e'_1,\dots\e'_n\}=\{\e_1,\dots\e_n\}T\,.
\end{equation}
   We say that these two bases have the same orientation
  if  the determinant of transition matrix from the first basis to the second one is positive:
  $\det T>0$.

  \noindent We say that the basis $\{\e'_1,\dots\e'_n\}$ has an orientation
  opposite to the orientation of the basis $\{\e'_1,\dots\e'_n\}$
...
\m
  ...
  One can see that orientation establishes  the equivalence relation in the set of all bases. Denote
                $$
...
\item  Is transitive, i.e. if $\{\e_1,\dots,\e_n\}\sim \{\e'_1,\dots,\e'_n\}$ and $\{\e'_1,\dots,\e'_n)\sim
\{\tilde \e_1,\dots, \tilde\e_n\}$, then $\{\e_1,\dots,\e_n\}\sim \{\tilde \e_1,\dots, \tilde\e_n\}$,
because if $T_1$ is transition matrix from the first basis $\{\e_1,\dots\e_n\}$ to the second
basis $\{\e'_1,\dots,\e'_n\}$ and $T_2$ is transition matrix from the second basis $\{\e'_1,\dots,\e'_n\}$
 to the third
basis $\{\tilde\e_1,\dots,\tilde\e_n\}$ then the transition matrix
$T$ from the first basis $\{\e_1,\dots,\e_n\}$ to the third
basis $\{\tilde\e_1,\dots,\tilde\e_n\}$ is equal to $T=T_2T_1$.
We have that $\det T=\det (T_2T_1)=\det T_2\cdot \det T_1>0$
because $\det T_1>0$ and $\det T_2>0$.

   \end{itemize}

Since it is equivalence relation the set of all bases is a union if disjoint equivalence classes.
Two bases are in the same equivalence class if and only if they have the same orientation.



One can see that there are exactly two equivalence classes.

Indeed let  $\{\e_1,\e_2\dots,\e_n\}$ be an arbitrary basis in $n$-dimensional vector space $V$.
...
\end{equation}
where one can easy see that the determinant for transition matrix $T_{\rm swap}$ is equal to $-1$.
...
We see that bases $\{\e_1,\e_2\dots,\e_n\}$ and $\{\e'_1,\e'_2\dots,\e'_n\}$ have opposite orientation. They
does not belong to the same equivalence class.

   Now consider in $V$ an arbitrary basis  $\{\tilde\e_1,\tilde\e_2\dots,\tilde\e_n\}$.
   For convenience call the initial basis $\{\e_1,\e_2\dots,\e_n\}$ the "I-st basis",
     call the basis  $\{\e'_1,\e'_2\dots,\e'_n\}$ the "II-nd basis" and
    call a new arbitrary basis  $\{\tilde\e_1,\tilde\e_2\dots,\tilde\e_n\}$ a "III-rd basis".
   Show that the III-rd basis and the I-st basis have the same orientation or
   the III-rd basis and the II-nd basis have the same orientation,
   i.e. the third basis belongs to the equivalence class of the first basis or it belongs
   to the equivalence class of the second basis. Thus we will show that there are exactly two equivalence classes.


   Let $T_1$ be  transition matrix from the I-st basis to the III-rd basis,i.e.
   from the basis $\{\e_1,\e_2\dots,\e_n\}$) to the basis
   $\{\tilde\e_1,\tilde\e_2\dots,\tilde\e_n\}$.
  Let $T_2$ be  transition matrix from II-nd basis to the III-rd basis,i.e.from
  the basis $\{\e'_1,\e'_2\dots,\e'_n\}$ to the basis
   $\{\tilde\e_1,\tilde\e_2\dots,\tilde\e_n\}$. We have:
                 \begin{equation*}
   \{\tilde\e_1,\tilde\e_2\dots,\tilde\e_n\}=\{\e_1,\e_2\dots,\e_n\}T_1,\,\,\,
    \{\tilde\e_1,\tilde\e_2\dots,\tilde\e_n\}=\{\e'_1,\e'_2\dots,\e'_n\}T_2
       \end{equation*}
Recall that $\{\e'_1,\e'_2\dots,\e'_n\}=\{\e_1,\e_2\dots,\e_n\}T_{\rm swap}$ with $\det T_{\rm swap}=-1<0$.

Hence
                \begin{equation*}\label{proof of two classes}
    \{\tilde\e_1,\tilde\e_2\dots,\tilde\e_n\}=\{\e'_1,\e'_2\dots,\e'_n\}T_2=
         \{\e_1,\e_2\dots,\e_n\}T_1T\,\Rightarrow T_2=T_1T_{\rm swap}\,.
       \end{equation*}

  We have $\det T_2=\det (T_1 T_{\rm swap})=\det T_1\det T_{\rm swap}=-\det T_1$.
     Hence $\det T_1$ and $\det T_2$ have opposite signs.
We come to


 {\bf Conclusion} If $\det T_1>0$ and $\det T_2<0$ , then  the  I-st and III-rd bases have the same orientation
  and the II-nd and III-rd bases  have opposite orientation.
  If $\det T_2>0$ and $\det T_1<0$ , then    the  II-nd and III-rd bases have the same orientation
  and the I-st and III-rd bases  have opposite orientation.

In other words if bases $\{\e_1,\e_2\dots,\e_n\}$, $\{\e'_1,\e'_2\dots,\e'_n\}$
have opposite orientation then an arbitrary basis  $\{\tilde\e_1,\tilde\e_2\dots,\tilde\e_n\}$
belongs to the equivalence class of the basis  $\{\e_1,\e_2\dots,\e_n\}$
or it belongs to the equivalence class of the basis  $\{\e'_1,\e'_2\dots,\e'_n\}$.
...
{\bf Example} (Orientation in two-dimensional space). Let $\{\e_x,\e_y\}$ be any basis in $\R^2$ and
 $\ac,\b$ are  arbitrary two vectors in $\R^2$.
...
are  linearly independent in $\R^2$)
 ...
 \begin{equation}\label{conditionofindepnednec}
                       \det T\not=0\,.
                    \end{equation}
 We see that
  the  basis  $\{\ac,\b\}$ has the same orientation as the basis $\{\e_x,\e_y\}$ if
              \begin{equation}\label{conditionofsameorientation}
                      \det T>0\,.
                   \end{equation}
We see that the basis $\{\ac,\b\}$ has the  orientation opposite to the orientation
of the basis $\{\e_x,\e_y\}$ if
        \begin{equation}\label{conditionofoppositeorientation}
                      \det T<0\,.
                    \end{equation}
...
{\bf Exercise} Show that bases $\{\e_x,\e_y\}$ and  $\{\e_y,\e_x\}$
have opposite orientation  (Transition matrix $T=\begin{pmatrix} 0&1\cr 1&0\cr\end{pmatrix}$ has determinant $-1$.).

{\bf Exercise} Bases $\{\e_x,\e_y\}$ and  $\{-\e_y,\e_x\}$
have the same orientation. (Transition matrix $T=\begin{pmatrix} 0&1\cr -1&0\cr\end{pmatrix}$ has determinant $1$.)

(There are plenty exercises in the Homework 3.)


Relations \eqref{conditionofsameorientation},\eqref{conditionofoppositeorientation}
           ...
           T=
                 ...
                 a_x   & b_x  &c_x \\
                  ...
                  \end{pmatrix}
                      $:
                      $$
\{\ac,\b,\bf \c\}=\{\e_x,\e_y,\e_z\}T= \{\e_x,\e_y,\e_z\}   \begin{pmatrix}
                 a_x   & b_x  &c_x \\
                  a_y   & b_y  &c_y\\
                   a_z   & b_z  &c_z\\
                  \end{pmatrix}
...
are  linearly independent)
 if and only if  transition matrix is not degenerate, i.e.
 \begin{equation}\label{conditionofindepnednec1}
                       \det T\not=0\,.
                    \end{equation}
 We see that
  the  basis  $\{\ac,\b,\c\}$ has the same orientation as the basis $\{\e_x,\e_y,\e_z\}$ if
              \begin{equation}\label{conditionofsameorientation1}
                      \det T>0\,.
                   \end{equation}
We see that the basis $\{\ac,\b,\c\}$ has the  orientation opposite to the orientation
...
{\bf Exercise} Show that bases $\{\e_x,\e_y,\e_z\}$ and  $\{-\e_x,\e_y,\e_z\}$
have opposite orientation but bases $\{\e_x,\e_y,\e_z\}$ and  $\{\e_y,\e_x,-\e_z\}$
have the same orientation.



  We say that Euclidean space is equipped with orientation if we consider in this space
  only orthonormal bases which have the same orientation.
...
{\small If two bases $\{\e_i\}$, $\{\e_{i'}\}$ have the same orientation then they can be transformed to each other by continuous
transformation, i.e. there exist one-parametric family of bases $\{\e_i(t)\}$ such that $0\leq t\leq 1$
and $\{\e_i(t)\}\vert_{t=0}=\{\e_i\}$, $\{\e_i(t)\}\vert_{t=1}=\{\e_{i'}\}$. (All functions $\e_i(t)$ are continuous)
In the case of three-dimensional space the following statement is true (Euler Theorem):
...
under rotation around the axis.}}
...
{\footnotesize
 \subsection { $^\dagger$Linear operator in $\E^3$ preservinig orientation is a  rotation}

\subsubsection {Linear operators and transition matrices}

Let $\{\e_1,\dots,\e_n\}$ be a basis in $n$-dimensional vector space $V$. Then one can see that
an arbitrary $n\times n$ matrix $A=||a_{ij}||$ defines a linear operator (with respect to this basis)
in the following way:
             \begin{equation}\label{linearoperatorin terms oftransition matrix1}
...
\begin{equation}\label{linearoperatorin terms oftransition matrix2}
 \hat A\x=(\e_1,\e_2,\dots,\e_n)\cdot A\cdot
 \begin{pmatrix}
 x^1\cr x^2\cr \dots \cr x^n\cr
 \end{pmatrix}=\sum_{i=1}^n\e'_i x^i=\sum_{i,k=1}^n\e_k a_{ki} x^i\,.
             \end{equation}



  Let $P$ be a linear operator in vector space $\R^n$.
   Let  $\{\e_1,\e_2,\dots,\e_n\}$ be an arbitrary  basis
  in $\R^n$. Considering the action of $P$ on basis vectors we come to vectors
   $(\e_1',\e_2',\dots,\e'_n)$:
   \begin{equation}\label{linearoperator}
    \e_1'=P(\e_1),\e_2'=P(\e_2)\dots, \e_n'=P(\e_n)
\end{equation}

  If operator $P$ is non-degenerate ($\det P\not=0$) then  ordered $n$-tuple
    $\{\e_1',\e_2',\dots,\e'_n\}$
   is a basis too.

   Non-degenerate linear operator maps the basis to another basis.


   {\bf Definition}.  Let  $\{\e_1,\e_2,\dots,\e_n\}$ be an arbitrary  basis
  in $\R^n$.  Consider the basis  $\{\e_1',\e_2',\dots,\e'_n\}$, where $\e_i'=P(\e_i)$
    We say that non-degenerate linear operator $P$ ($\det P\not=0$)
    preserves  orientation if bases
   $\{\e_1,\e_2,\dots,\e_n\}$ and $\{\e_1',\e_2',\dots,\e'_n\}$, where $\e_i'=P(\e_i)$
    have the same orientation. In this case $\det P>0$.

  We say that linear operator $P$ changes  orientation if bases
   $\{\e_1,\e_2,\dots,\e_n\}$ and  $\{\e_1',\e_2',\dots,\e'_n\}$ have opposite orientation.
   In this case $\det P<0$.


\m

 It is easy to see that this definition is correct: The property of operator $P$ to preserve
 orientation does not depend on choosing a basis.
   If bases
 $\{\e_1,\e_2,\dots,\e_n\}$ and  $\{\e_1',\e_2',\dots,\e'_n\}$, where $\e_i'=P(\e_i)$
 have the same (opposite) orientation, then for an another  basis
 $\{\f_1,\f_2,\dots,\f_n\}$ in $\R^n$, the bases
 $\{\f_1,\f_2,\dots,\f_n\}$ and  $\{\f_1',\f_2',\dots,\f'_n\}$, where $\f_i'=P(\f_i)$
  have the same (opposite) orientation also.

\m

In other words we say that non-degenerate linear operator $P$ preserves orientation if it maps
vectors of an arbitrary  basis to the vectors of another basis which have the same orientation as
an initial basis. We say that non-degenerate linear operator $P$ changes orientation if it maps
vectors of an arbitrary  basis to the vectors of another basis which have the orientation opposite
an orientation of initial basis.

\m

{\bf Example} Let $\{\e_x,\e_y,\e_z\}$ be an orthonormal basis in $\E^3$. Consider linear operator
  $P$ such that
                  $$
        P(\e_x)=\e_y, P(\e_y)=-\e_x, P(\e_z)=\e_z\,.
                  $$
  This operator maps orthonormal basis  $\{\e_x,\e_y,\e_z\}$ to the basis $\{\e_y,-\e_x,\e_z\}$ which is
  orthonormal
  too. Both bases have the same orientation. Hence the operator $P$ is linear operator preserving orientation.
  In this case it is orthogonal operator, because it maps orthonormal basis to the orthonormal one.
  One can see that $P$ is rotation operator: Under the action of operator $P$ vectors in $\E^3$
  rotate on the angle $\pi\over 2$ about the axis $Oz$. The vectors $\lambda \e_z$
  collinear (proportional) to the vector $\e_z$.
  are eigenvectors of this operator: $P\e_z=\e_z$. The axis is a line spanned by the vector $\e_z$.

\smallskip

  One can show that in Euclidean vector space $\E^3$ every orthogonal operator which
  preserves orientation is a rotation.

  \bigskip

  {\bf Theorem} (Euler Theorem). {Let $P$ be an linear orthogonal operator in $\E^3$ preserving orientation.
  Then it is a rotation operator about some axis passing through the origin.}

    \m
    (The proof of this theorem see in the solutions of Homework 3, Exercise 7)

     How to find an axis of rotation? Vectors which belong to axis (starting at origin) are {\it eigenvectors} of $P$.
     They all are proportional each other. Eigenvalue of these vectors is equal to $1$--- Rotation
     does not change the vectors which belong to axis. Hence

\m

 \noindent   {\bf  Claim}  {To find an axis we have to find eigenvector of the operator $P$ with eigenvalue $1$.}

\smallskip

   In the example above
     vector $\e_z$ was the eigenvector of the operator $P$. Consider more interesting example:

     \m

{\bf Example} Let $\{\e_x,\e_y,\e_z\}$ be an orthonormal basis in $\E^3$. Consider linear operator
  $P$ such that
\begin{equation}\label{rotation1}
    P(\e_x)=\e_z, P(\e_y)=-\e_y, P(\e_z)=\e_x\,.
\end{equation}
 This operator maps orthonormal basis  $\{\e_x,\e_y,\e_z\}$ to the basis $\{\e_z,-\e_y,\e_x\}$ which is
  orthonormal
  too. Both bases have the same orientation. Hence the operator $P$ is linear orthogonal
  operator preserving orientation.  According to the Euler Theorem it is a rotation operator about an axis.
  Find this axis. Let vector $\N$ (starting at origin) belongs to the axis. Then
\begin{equation}\label{eigenvector1}
    P\N=\N\,.
\end{equation}
$\N$ is eigenvector of the operator $P$. Its eigenvalue is equal to $1$.
To find an axis of rotation \eqref{rotation1} we have to find an eigenvector \eqref{eigenvector1}.
 It is easy to see that vector $\e_x+\e_z$ obeys the condition \eqref{eigenvector1}:
               $$
              P(\e_x+\e_z)=\e_x+\e_z
               $$
We see that eigenvector of $P$ is an arbitrary vector proportional (collinear) to the vector $\e_x+\e_z$.
These vectors span the line $\lambda (\e_x+\e_y)$--axis of rotation.
 We see that the axis of rotation is the line spanned by the eigenvectors which is a bisectrix
  of the angle between $Ox$ and $Oz$ axis.
}
...
Now we give a definition of vector product of vectors in $3$-dimensional Euclidean space equipped with orientation,
i.e. we fix an equivalence class of orthonormal bases with the same orientation.
 Recall that it suffices to fix an arbitrary orthonormal basis $\{\e_x,\e_y,\e_z\}$.
  Then an equivalence class is defined as the set of all orthonormal bases which have the same orientation
  as the basis $\{\e_x,\e_y,\e_z\}$


\m

 Let $\E^3$ be three-dimensional oriented Euclidean space, i.e. Euclidean space equipped with an equivalence class
 of bases with the same orientation. To define the orientation it suffices to consider just one orthonormal basis
 $\{\e_1,\e_2,\e_3\}$. Then the equivalence class of the bases is a
 set of all bases which have the same orientation as the basis $\{\e_1,\e_2,\e_3\}$.


  By default we suppose that orthonormal basis $\{\e_x,\e_y,\e_z\}$ belongs to the equivalence class
  of bases defining orientation of $\E^3$.


  Let $\E^3$ be three-dimensional oriented Euclidean space.
...
\end{equation}
\item
 If the  ordered triple of the vectors $\{\x,\y, {\bf z}\}$, where ${\bf z}=\x\times \y$ is a basis,
  then this basis and an orthonormal basis $\{\e_x,\e_y,\e_z\}$ in $\E^3$ have the same orientation:
\begin{equation}\label{vectorproductaxiom5}
\{\x,\y, {\bf z}\}=\{\e_x,\e_y,\e_z\}T, \,\, \hbox {where for transition matrix $T$, $\det T>0$.}
\end{equation}
...
Thus we will prove the uniqueness and existence.


Let $\{\e_x,\e_y,\e_z\}$ be an {\it arbitrary } orthonormal basis of oriented Euclidean space $\E^3$
which belongs to the equivalence class of bases defining orientation.
...
\m
  ...
  We see that the operation $L(\x,\y)=\x\times \y$ which obeys all the axioms
...
On the other hand  using the properties of determinant and the fact that
  ...
  by the formula \eqref{defofvectorproduct} in  an arbitrary orthonormal basis  (with rightly chosen orientation).
...
from the equivalence class of bases defining orientation.
  ...
  the sign. (See also the Homework 4)
...
{\bf Remark}  It is useful sometimes to consider area of parallelogram not as a positive number but
...
S(\ac,\b)=
              ...
              \det\begin{pmatrix}
                  ...
                  \right|\,.
\end{equation}
{\bf Remark} in the formulae above calculation depend (up to a sign) on the choise of orientation.

We come to very beautiful formulae for relation between determinant of $2\times 2$ matrix area of parallelogram
and vector product.

One can deduce this relation in other way:
...
$A(\ac,\b)$ and these are the conditons which define the determinant of the $2\times 2$ matrix.
...
\left(\ac,\left[\b\times \c\right]\right)=\begin{pmatrix}
                a_x  &a_y  &a_z \\
                  ...
                  \end{pmatrix}\,.
\end{equation}
{\bf Remark} The volume of the parallelepiped if considered as a positive number
...
\section {Differential forms in $\E^2$ and $\E^3$}

\subsection {Tangent vectors, curves, velocity vectors on the curve}

Tangent vector is a vector $\v$ applied at the given point $\pt\in \E^3$.
 ...
 $T_\pt(\E^3)$.
  ...
  One can consider {\it vector field} on $\E^3$, i.e.a function which assigns
  to every point $\pt$ vector $\v(\pt)\in T_\pt(\R^3)$.


It is instructive to study the conception of tangent vectors and vector fields
on the curves and surfaces embedded in $\E^3$. We begin with curves.
...
{\bf Remark}
  We consider only smooth and regular curves. Curve ${\bf r}(t)$ $=(x^1(t),\dots,x^n(t))$ is called smooth
  ...
  By default we consider simple curves, i.e. curves which have no intersection points.
...
with different velocity.  Two equivalent curves have different orientation
...
only for $\E^2$ or $\E^3$.
  Compulsory material for differential forms
is only for $\E^2$, $\E^3$}.}
...
where components $\w_i(\r)=\w(\r,\e_i)$.
  ...
  an arbitrary differential $1$-form $\w$ can be expanded over the basis forms\eqref{basicforms}
   $$
   \w=\w_1(x^1,\dots,x^n)dx^1+\w_2(x^1,\dots,x^n)x^2+\dots+\w_n(x^1,\dots,x^n)dx^n\,.
     $$
...
\bigskip
 ...
 To introduce this new notation we need a short recalling of derivational derivative of functions.
...
\end{equation}
 ...
 Vector $\e_x$ we will denote sometimes by $\p_x$,
vectors $\e_y, \e_z$ by $\p_y, \p_z$ respectively.
The symbols  $\p_x,\p_y, \p_z$ correspond to partial derivative with respect to coordinate
...
respectively
    ...
    {\p_x}={\p r\over \p x}\p_r+{\p \varphi\over \p x}\p_\varphi={x\over r} \p_r-{y\over r^2} \p_\varphi\,\,
                      \end{equation*}
and
                     \begin{equation}\label{polarbasicfields}
...
According to calculations in previous example we have that
           $$
...
{\it In Cartesian coordinates}: $\A=r{\p\over \p r}=x{\p\over \p x}+y{\p\over \p y}$. Hence
  ...
  Or using the fact that $\p_\A F=dF$ we have that $\p_\A (x^4-y^4)=d(x^4-y^4)(\A)=$
        ...
        (4x^3dx-4y^3dy)\left(x{\p\over \p x}+y{\p\over \p y}\right)=
          ...
          4x^4-4y^4\,.
            $$
In polar coordinates $F=x^4-y^4=(x^2-y^2)(x^2+y^2)=r^2(r^2\cos^\varphi-r62\sin62\varphi)=r^4\cos 2\varphi$.
...
\begin{equation}\label{changingtheparameterisationforintegral1}
    \int_C \w=\int_C' \w,\quad \hbox{if orientaion is not changed, i.e. if $t'(\tau)> 0$}
\end{equation}
and
\begin{equation}\label{changingtheparameterisationforintegral2}
    \int_C \w=-\int_C' \w,\quad \hbox{if orientaion is  changed, i.e. if $t'(\tau)< 0$}
\end{equation}
\m
...
Now consider integrals of the same form $\w$ over three curves which differ by reparameterisation.

\m

{\bf Example}
Consider $1$-form $\w=xdy-ydx$ and curve $C$---- upper half of the circle $x^2+y^2=R^2$, ($y\geq 0$).

We have the image of the curve not the parameterised curve. Consider different parameterisations of this curve:
           $$
           \r_1(t)\colon\,\,
              \begin{cases}
              x=R\cos t\cr
         ...
         \r_3(t)\colon\,\,
              ...
              x=t\cr
              y=\sqrt {R^2-t^2}\cr
              \end{cases}, 0\leq t\leq R,,\qquad
               \end{equation}

All these curves are the same image. If $\Omega=1$ the second curve coincides with the first one.
First and second curve have the same orientation (reparameterisation $t\mapsto \Omega t$)
The third curve have orientation opposite to first and second (reparameterisation
...
Note that these integrals is much nicer to calculate in polar coordinates: Recall that in polar coordinates
          $$
  \w=xdy-ydx=r\cos \varphi d(r\cos \varphi)-r\sin \varphi d(r\cos \varphi)=r^2d\varphi
          $$
      Hence
          $$
\int_{C_1} \w=\int_0^\pi(r^2\varphi_t)dt=\pi R^2,\,\,\int_{C_2} \w=\int_0^{\pi/\Omega}
(r^2\varphi_t)dt=\pi R^2\,.
          $$
 For the third parameterisation:
           $$
     \int_{C_3} \w=\int_0^{R}(xy_t-yx_t)dt=\int_0^{1}\left(
...
Then the integral of this form over an arbitrary curve   $C\colon\quad \r=\r(t)\quad t_1\leq t\leq t_2$
     is defined by the ending and starting point of the curve:
...
much more efficient way noting that
it is an exact form:
...
\m
 ...
 {\bf Example}. Show that the integral of $1$-form $\w=x^5dy+5x^4ydx$ over the ellipse $x^2+{y^2\over 9}=1$\,.
...
\subsection {$0$-forms (functions) ${\buildrel d\over \longrightarrow}$
...
\section {Curves in $\E^3$. Curvature}
...
$\ac={d^2\r(t)\over dt^2}$. For curve in $\r=\r(t)$ $\E^n$ we have
          $$
...
For this purpose consider the equation for speed:
 ...
 {d|\v|^2\over dt}={d\over dt}(\v(t),\v(t))=2(\v(t),\ac(t))=|\v(t)||\ac(t)|\cos\theta (t)=
 |\v(t)||\ac_{tangent}(t)|\,,
\end{equation}
...
We observed this phenomenon already when we considered the moving along the curve with different velocities
(see \eqref{anglebetweenvelocityandacceleration1}, \eqref{anglebetweenvelocityandacceleration2} and
...
\begin{equation}\label{generalformulaforcurvature2}
    k={{|\ac_{\perp}(t)|\over |\v(t)|^2}}={\hbox{Area of parallelogram formed by the vectors $\ac,\v$}\over |\v|^3},
\end{equation}
...
Do now the next step: Note that
       $$
       {|\ac_{\perp}(t)|\over |\v|^2}={|\ac_{\perp}(t)|\cdot|\v|\over |\v|^3}=
       $$
\begin{equation}\label{modulusofcentripetal}
{\hbox{Area of parallelogram spanned by the vectors $\ac,\v$}\over |\v|^3}=={|\ac\times \v|\over |\v|^3}\,.
\end{equation}
$$
={|\ac\times \v|\over |\v|^3}
$$
The last formula is practical. We already know how to calculate  area of parallelogram
...
Hence for $\E^3$ formula for curvature is:

\begin{equation}\label{Gebformforcurvn=3}
   k={{|\v(t)\times \ac(t)|\over |\v(t)|^3}},\qquad (\hbox{if curve is in $\E^3$})
\end{equation}

In $\E^2$ formula for curvature is
\begin{equation}\label{Gebformforcurvn=21}
    k={{|\v(t)\times \ac(t)|\over |\v(t)|^3}}={|v_xa_y-v_ya_x|\over (v_x^2+v_y^2)^{3\over 2}}
    \qquad (\hbox{if curve is in $\E^2$})
\end{equation}
or more explicit formula:
\begin{equation}\label{Gebformforcurvn=22}
    k={|x_{t}y_{tt}-y_{t}x_{tt}|\over (x^2_{t}+y^2_{t})^{3\over 2}} \qquad (\hbox{if curve is in $\E^2$})
\end{equation}
\m
...
Consider examples of calculating curvature for curves in $\E^2$.
...
Applying  the formula \eqref{Gebformforcurvn=22} we come to
    ...
    {|R^2\cos^2 t+R^2\sin^2 t|\over (R^2\cos^2 t+R^2\sin^2 t)^{3\over 2}}={R^2\over R^3}={1\over R}
      $$
{\bf Example} Consider ellipse $${(x-x_0)^2\over a^2}+{(y-y_0)^2\over b^2}$$. Choose a parameterisation, e.g.
  $\r(t)\colon \begin{cases}x=x_0+a\cos t\\ y=y_0+b\sin t\end{cases}$,
   $0\leq t< 2\pi$.
...
\end{equation}
In particular we see that at the points $(x_0\pm a, y_0)$ curvature equals to
$k={ab\over b^3}={a\over b^2}$ and at the points  $(x_0, y_0\pm b)$ curvature equals to
$k={ab\over a^3}={b\over a^2}$.


  See other examples in Homework 7.



{\footnotesize
{\bf Example} For any function $f=f(x)$ one can consider its graph as not-parameterised curve $C_f$.
Calculate curvature of the curve $C_f$ at any point $(x,f(x))$.

One can choose parameteristaion:
$\r(t)\colon\quad \begin{cases}x=t\\ y=f(t)\end{cases}$.

Then  $\v(t)=\begin{pmatrix} 1\\ f'(t)\end{pmatrix}$,
...
It is interesting to consider this example when $F=uv$ we come to the surface,  {\it saddle}:
...
What about internal observer?
 ...
 We have
         $$
       \v={d\r(t)\over dt}={d\r(u(t),v(t))\over dt}=\dot u{\p r(u,v)\over \p u}+\dot v{\p r(u,v)\over \p v}=\dot u \r_u\,.
         $$
Hence the scalar product
 ...
 To understand how internal observer can calculate the length of the curve we have to introduce
    ...
    G_{uu}=(\r_u,\r_u),\,\,\,G_{uv}=(\r_u,\r_v)\,\,\,G_{vu}=(\r_v,\r_u),\,\,\G_{vv}=(\r_v,\r_v)
\end{equation}
...
\begin{pmatrix}
   ...
   \end{pmatrix}
\end{equation}
(See this example and other examples in Homework 5)
...
and $\r_v$, i.e. it is orthogonal to the surface.
 ...
 $\n(u,v)$ at the point $\r(u,v)$.
  (See other examples of calculating normal unit vector in the Homework 8)
...
\def\xip {{\boldsymbol \xi}}
  Let $M\colon \r=\r(u,v)$ be a surface and ${\bf L}(u,v)$ be an arbitrary (not necessarily unit normal)
...
\begin{equation}\label{propertyofshapeoperator1}
  S\colon T_\pt M\to T_\pt M\,\, \hbox{for every}\,\, \xip\in T_\pt M,
\qquad S(\xip)\in T_\pt M
\end{equation}
...
is normal vector field too. Later we will see why it is convenient (see the proof of the Proposition below).
   ...
   Show that the property  \eqref{propertyofshapeoperator1} is indeed obeyed, i.e.
   vector $\xip'=S(\xip)$ is tangent to surface.  Consider derivative of scalar product $(\n,\n)$
with respect to the vector field $\xip$. We have that $(\n,\n)=1$. Hence
                 $$
   \p_\xip(\n,\n)=0=\p_\xip (\n,\n)=(\p_\xip \n,\n)+(\n,\p_\xip \n)=2(\p_\xip \n,\n)\,.
              $$
   Hence $(\p_\xip \n,\n)=-(S(\xip),\n)=-(\xip',\n)=0$, i.e. vector
$\p_\xip \n=-\xip'$ is orthogonal to the vector $\n$. This means that vector $\xip'$ is tangent to the surface.


Examples of shape operator see in the Homeworks.



{\footnotesize
\bigskip We show now that normal acceleration of a curve on the surface and normal curvature are expressed in terms
of shape operator.
...
\end{itemize}
 ...
 E.g. if  in a given basis  a shape operator is given by the matrix
...
$\n(\theta,\varphi)={\r(\theta,\varphi)\over R}=\begin{pmatrix}
                                         ...
                                         This is evident geometrically and
