...
% here I have to cut 4-th year material and add shape operator
...
\maketitle
  ...
  \centerline { Manchester, 4  May  2018}
...
An $n$-dimensional manifold is a space such that in a vicinity of an
arbitrary  point
one can consider local coordinates $\{x^1,\dots,x^n\}$. 
...
taking values also in $\R^n$:
              \begin{equation}\label{changingofcoordinates}
             \begin{cases}
...
Consider circle $x^2+y^2=R^2$ of radius $R$ in $\E^2$.

One can consider on the circle different local coordinates
...
between these different coordinates.
...
Consider sphere $x^2+y^2+z^2=R^2$ of radius $a$ in $\E^3$.

One can consider on the sphere different local coordinates
...
except north pole;  stereographic 
coordinates $(u',v')$ are defined elsewhere
except south pole. One can consider transition function
...
the space of vectors tangent to the manifold at the point $M$.
 ...
 Tangent vector $\A(x)=A^i(x){\p\over \p x^i}$.
Under changing of coordinates it transforms as follows:
             $$
         \A=A^i(x){\p\over \p x^i}=
A^i(x){\p x^{m'}(x)\over \p x^i}{\p\over \p x^{m'}}=
         A^{m'}(x'(x)){\p\over \p x^{m'}}\,.
             $$
  Hence

            \begin{equation}\label{transformofvector}
                   A^{i'}(x')={\p x^{i'}(x)\over \p x^i}A^i(x)\,.
            \end{equation}
...
(for every point $\pt$ on manifold $M$)---
         ...
         One-form (covector) $\w=\w_i(x)dx^i$ transforms as follows
            $$
      \w=\w_m (x)dx^m=\w_m {\p x^m(x')\over \p x^{m'}}
               dx^{m'}=\w_{m'}(x')dx^{m'}\,.
        $$
      Hence
...
$\begin{pmatrix}p \cr q\cr\end{pmatrix}=$
$\begin{pmatrix}1 \cr 0\cr\end{pmatrix}$,
and $1$-form \eqref{transformofoneform} is nothing but
tensor field of valency 
$\begin{pmatrix}p \cr q\cr \end{pmatrix}=$
...
tensor of the type $\begin{pmatrix} 1\cr 2\cr\end{pmatrix}$) then
            \begin{equation}\label{ruleoftransformfor2tensor2}
                S^{m'}_{i'k'}=
   {\p x^{m'}\over \p x^m}
   {\p x^i\over \p x^{i'}}
   {\p x^k\over \p x^{k'}}
...
Transformations formulae
\eqref{transformofvector}---\eqref{ruleoftransformfor2tensor2}  define
vectors, covectors and in generally any tensor fields in components. 
...
for that index.
 ...
 Looking  on righthand sides of equations 
\eqref{transformofvector}---\eqref{ruleoftransformfor2tensor2} 
we see that Einstein rules
help essentially to write down formulae correctly. 
...
\footnote{ One can show that arbitrary 
$2$-dimensional 
Riemannian manifold is conformally Euclidean, i.e. in a vicnitiy
of arbitrary point there exist conformal coordinates.}. 
...
\eqref{conformaleuclideandef} has to be positive.
  It is convenient sometimes to denote it as $\s(x)=e^{\l(x)}$,
                \begin{equation}\label{conformaleuclideandef1}
                  G=\s(x)\delta_{ik}dx^idx^k=e^{\l(x)}\delta_{ik}dx^idx^k\,,
                \end{equation}
...
{\l(x)\delta_{ik}X^iY^k\over \sqrt {\l(x)\delta_{ik}(x)X^iX^k}
    \sqrt {\l(x)\delta_{ik}(x)Y^iY^k}}=
{\sum_kX^kY^k\over \sqrt {\sum_k(x)X^kX^k}
    \sqrt {\sum_kY^kY^k}}\,.
                                  \end{equation}

  It is instructive to recall the example    
considered in subsection \ref{lengthandangle}),
...
locally conformally Euclidean in a vicnity of a point
$\pt$. 
...
vector $\r_u\in T_pM$, as a  velocity vector
for the curve $u=u_0+t,v=v_0$, where $(u_0,v_0)$ 
are coordinates of the point $\pt$,
and Internal Observer denotes this vector $\p_u$. 
Respectively the basis vector $\r_v\in T_pM$ 
for an Internal Observer, is velocity vector
for the curve $u=u_0,v=v_0+t$, where $(u_0,v_0)$ 
are coordinates of the point $\pt$,
...
Volume form \eqref{volumelement} is invariant with respect 
to changing of coordinates\footnote{see 
paragraph \ref{invariance} in Appendices}.
In the next paragraph we will give motivation of
this formula.
...
\subsubsection {Definition.  Geodesic on 
...
One can see that if Riemannian metric $G=G_{ik}du^idv^k$ have constant coefficients
  ...
  then Riemannian metric is equal to $G=a^2d\varphi^2+dh^2$ and we come to equations:
    ...
    \end{cases} \Rightarrow
     \begin {cases}
    {\varphi(t)\over dt}=\varphi_0+\Omega t\cr
    {d h(t)\over dt}=h_0+tc \cr
    \end{cases}\,.
       \end{equation}
...
In the new parameterisation the velocity vector
  ...
  c(t)\v(t)=\A(t(\tau))$ and
     $$
{d \v'\over d\tau}=\nabla_{\v'}\v'=
\nabla_{t_\tau\v}(t_\tau\v)(t_\tau \v)-
t_\tau\nabla_{\v}(t_\tau\v)(c(t) \v)=
t_\tau\nabla_\v\A=0
     $$
...
\subsubsection {Weingarten operator on surfaces and Gaussian curvature}
...
\end{equation}
(This matrix is the matrix of Weingarted operator in the basis
   $(\r_u,\r_v)$.)
...
curvatures for cylinder and sphere.}
...
Gaussian curvature of the surface can be expressed in terms of induced Riemannian metric. It
...
Now notice that  left hand side od this equation defining 
Gaussian curvature $K(\pt)$ depends only on Riemannian
metric on the surface $C$. Indeed numerator of LHS is 
...
Now applu formula \eqref{theoremofrotationonangle}:
